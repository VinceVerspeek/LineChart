...
const render = data => {
    ...
    const yScale2 = d3.scaleLinear()
        .domain([0, d3.max(data, a => a["Economy (GDP per Capita)"],
        b => b["Freedom to make life choices"],
        c => c["Healthy life expectancy"],
        d => d["Generosity"],
        e => e["Trust (Government Corruption)"]) + 0.3])
        .range([innerHeight, 0])
        .nice();
    ...
    let dots = g.selectAll('dot').data(data)
    .enter().append('circle')
        .attr('cy', innerHeight)
        .attr('cx', xHappDots)
        .attr('r', 8)
    ...
    let ecoDots = g.selectAll('circle2').data(data)
    .enter().append('circle')
        .attr('class', 'circle2')
        .attr('cy', innerHeight)
    ...
    let freeDots = g.selectAll('circle3').data(data)
    .enter().append('circle')
        .attr('class', 'circle3')
    ...
    let healthDots = g.selectAll('circle4').data(data)
    .enter().append('circle')
        .attr('class', 'circle4')
        .attr('cy', d => innerHeight)
        .attr('cx', d => xScale(d["Year"]))
    ...
    let trustDots = g.selectAll('circle6').data(data)
    .enter().append('circle')
        .attr('class', 'circle6')
        .attr('cy', d => innerHeight)
        .attr('cx', d => xScale(d["Year"]))
    ...
    function handleMouseOverEco(d, i) {
        ...
        // Determine text of tooltip
        div.html(i["Year"] + "<br/>" + numberFormat(i["Economy (GDP per Capita)"]))
            .style("left", xText + "px")
            .style("top", yText + "px");
    ...
    function handleMouseOverFree(d, i) {
        ...
        // Determine text of tooltip
        div.html(i["Year"] + "<br/>" + numberFormat(i["Freedom to make life choices"]))
            .style("left", xText + "px")
            .style("top", yText + "px");
    ...
    function handleMouseOverHealth(d, i) {
        ...
        // Determine text of tooltip
        div.html(i["Year"] + "<br/>" + numberFormat(i["Healthy life expectancy"]))
            .style("left", xText + "px")
            .style("top", yText + "px");
    ...
    function handleMouseOverTrust(d, i) {
        ...
        // Determine text of tooltip
        div.html(i["Year"] + "<br/>" + numberFormat(i["Trust (Government Corruption)"]))
            .style("left", xText + "px")
            .style("top", yText + "px");
    ...
    // Legend circles
    g.append("circle").attr("cx",innerWidth - legendOffset).attr("cy",130).attr("r", 6).style("fill", "steelblue")
    g.append("circle").attr("cx",innerWidth - legendOffset).attr("cy",160).attr("r", 6).style("fill", "red")
    g.append("circle").attr("cx",innerWidth - legendOffset).attr("cy",190).attr("r", 6).style("fill", "orange")
    g.append("circle").attr("cx",innerWidth - legendOffset).attr("cy",220).attr("r", 6).style("fill", "violet")
    g.append("circle").attr("cx",innerWidth - legendOffset).attr("cy",250).attr("r", 6).style("fill", "teal")
    g.append("circle").attr("cx",innerWidth - legendOffset).attr("cy",280).attr("r", 6).style("fill", "turquoise")

    // Legend text
    g.append("text").attr("x", innerWidth - legendOffset + 25).attr("y", 130).text(scores.happiness.type)
        .style("font-size", legendFontSize).attr("alignment-baseline","middle")
    g.append("text").attr("x", innerWidth - legendOffset + 25).attr("y", 160).text(scores.economy.type)
        .style("font-size", legendFontSize).attr("alignment-baseline","middle")
    g.append("text").attr("x", innerWidth - legendOffset + 25).attr("y", 190).text(scores.freedom.type)
        .style("font-size", legendFontSize).attr("alignment-baseline","middle")
    g.append("text").attr("x", innerWidth - legendOffset + 25).attr("y", 220).text(scores.health.type)
        .style("font-size", legendFontSize).attr("alignment-baseline","middle")
    g.append("text").attr("x", innerWidth - legendOffset + 25).attr("y", 250).text(scores.generosity.type)
        .style("font-size", legendFontSize).attr("alignment-baseline","middle")
    g.append("text").attr("x", innerWidth - legendOffset + 25).attr("y", 280).text(scores.trust.type)
        .style("font-size", legendFontSize).attr("alignment-baseline","middle")
}